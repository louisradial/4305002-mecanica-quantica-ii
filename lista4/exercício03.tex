...
\begin{proof}[Resolução]
   ...
   Como o vácuo é autoestado de paridade, temos
   \begin{equation*}
      \Pi A_{\vetor{k}, \lambda} \Pi = \eta_\Omega A_{-\vetor{k}, -\lambda},
   \end{equation*}
   e então
   \begin{align*}
      \Pi H_a \Pi &= \eta_{\Omega}\int_{\mathbb{R}^3} \dln3k \sum_{m = -j}^{j} \sum_{\lambda = -s_a}^{s_a} \left[f_{\lambda}(k) D^j_{m\lambda}(\vetor{k}) A_{-\vetor{k},-\lambda}\herm{\xi}_m + \mathrm{h.c.}\right]\\
                  &= \eta_{\Omega}\int_{\mathbb{R}^3} \dln3k \sum_{m = -j}^j \sum_{\lambda = -s_a}^{s_a} \left[f_{-\lambda}(k) D^j_{m {-\lambda}}(-\vetor{k}) A_{\vetor{k},\lambda} \herm{\xi}_m + \mathrm{h.c.}\right].
   \end{align*}
   Dessa forma, se a interação é invariante por paridade, \(\Pi V \Pi = \eta_V V,\) devemos ter
   \begin{equation*}
      f_{-\lambda}(k) D^j_{m {-\lambda}}(-\vetor{k}) = \eta f_{\lambda}(k) D^j_{m \lambda}(\vetor{k})
      \quad\text{e}\quad
      g_{-\lambda}(k) D^j_{m {-\lambda}}(-\vetor{k}) = \eta g_{\lambda}(k) D^j_{m \lambda}(\vetor{k}),
   \end{equation*}
   onde absorvemos todos os autovalores de paridade em \(\eta = \eta_V \eta_\Omega.\) Utilizando ângulos de Euler, temos
   \begin{align*}
      D^{j}_{m {-\lambda}}(-\vetor{k}) &= D^{j}_{m{-\lambda}}(\pi + \phi, \pi - \theta,0)\\
                                       &= e^{-i(\pi + \phi)m} d^{j}_{m{-\lambda}}(\pi - \theta)\\
                                       &= (-1)^j e^{-i\phi m} d^{j}_{m\lambda}(\theta)\\
                                       &= (-1)^j D^j_{m \lambda}(\vetor{k}),
   \end{align*}
   logo a restrição por invariância de paridade é
   \begin{equation*}
      f_{-\lambda}(k) = (-1)^j \eta f_\lambda(k)\quad\text{e}\quad g_{-\lambda}(k) = (-1)^j \eta g_{\lambda}(k).
   \end{equation*}

   Em particular, temos \(\abs{f_{\lambda}(k)} = \abs{f_{-\lambda}(k)}\) e \(\abs{g_{\lambda}(k)} = \abs{g_{-\lambda}(k)}.\) Este resultado motiva o estudo do caso em que \(s_a = s_b = \frac12,\) já que esta condição se reduz a
   \begin{equation*}
      f_\lambda(k) = e^{i\chi^{(a)}_\lambda} f(k),\quad\text{e}\quad g_{\lambda}(k) = e^{i\chi^{(b)}_\lambda} g(k),
   \end{equation*}
   onde \(\chi_\lambda \in \mathbb{R}\). No caso em que a partícula incidente tem momento angular orbital nulo, temos \(j = \frac12\) e então a matriz \(K\) assume apenas valores na diagonal,
   \begin{equation*}
      K_{mm'} = \left\{E - E_0  - \int_{\mathbb{R}^3} \dln3k \left[\frac{\abs{f(k)}^2}{E - E_a(\vetor{k}) + i \epsilon} + \frac{\abs{g(k)}^2}{E - E_b(\vetor{k}) - \mu + i \epsilon}\right]\right\}\delta_{mm'} = \left[E - E_0 - \Sigma(E)\right] \delta_{mm'},
   \end{equation*}
   se assemelhando ao caso de spin nulo \(s_a = s_b = 0.\) O elemento de matriz de espalhamento abaixo do limiar inelástico é agora
   \begin{equation*}
      M_{fi} = -4\pi^2 m_a \delta_{\lambda_i \lambda_f} \frac{f(k_i) f(k_f)}{E - E_0 - \Sigma(E)},
   \end{equation*}
   e então o acoplamento entre os diferentes estados de helicidade é trivializado. Para \(j > \frac12,\) podemos escrever a matriz \(K\) como
   \begin{equation*}
      K_{mm'} = \left[E - E_0 - \Sigma(E)\right]\delta_{mm'} + \int_{\mathbb{R}^3} \dln3{k} \left[\frac{\abs{f(k)}^2}{E - E_a(\vetor{k}) + i \epsilon} + \frac{\abs{g(k)}^2}{E - E_b(\vetor{k}) - \mu + i \epsilon}\right]\sum_{\abs{\lambda} > \frac12}^j D^j_{m' \lambda}(\vetor{k}) D^j_{m\lambda}(\vetor{k})^*
   \end{equation*}
   e vemos que ainda há acoplamento entre os estados de helicidade.
\end{proof}

...
\begin{align}
    S(\rho^{(1)} \otimes \rho^{(2)}) &= -\sum_i \sum_j p^{(1)}_i p_j^{(2)} \ln(p^{(1)}_i p^{(2)}_j)\\
                                     ...
                                     &= S(\rho^{(1)}) + S(\rho^{(2)}).
\end{align}
A subaditividade da entropia de von Neumann estabelece que ao tomar o traço parcial de cada componente do sistema perdemos informação.
...
\begin{align}
    \entropy{\rho}{\rho^{(1)}\otimes \rho^{(2)}} &= - \Tr( \rho \ln \rho^{(1)} \otimes \rho^{(2)}) - S(\rho)\\
                                                 &= -\Tr(\rho \ln \rho^{(1)}) - \Tr(\rho \ln \rho^{(2)}) - S(\rho)\\
                                                 &= S(\rho^{(1)}) + S(\rho^{(2)}) - S(\rho),
\end{align}
onde usamos que podemos escrever \(\rho = \rho^{(1)} \otimes \rho^{(2)} + C\) com \(\Tr(C) = \Tr_1(C) = \Tr_2(C) = 0.\) Neste caso, a entropia relativa mede a diferença de entropia ao se tomar o traço parcial do estado composto. Ainda, para estados \(\rho\) e \(\sigma\) do sistema composto, vale
\begin{equation}
    \entropy{\rho^{(1)}}{\sigma^{(1)}} \leq \entropy{\rho}{\sigma},
\end{equation}

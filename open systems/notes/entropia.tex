...
\end{equation}
onde \(\rho^{(1)} = \Tr_2 \rho\) e \(\rho^{(2)} = \Tr_1 \rho\) são os estados reduzidos dos subsistemas. A subaditividade da entropia de von Neumann estabelece que ao tomar o traço parcial de cada componente do sistema perdemos informação. A igualdade se dá no caso em que o estado \(\rho\) não apresenta correlação, \(\rho = \rho^{(1)} \otimes \rho^{(2)},\) já que
\begin{align}
    S(\rho^{(1)} \otimes \rho^{(2)}) &= -\sum_i \sum_j p^{(1)}_i p_j^{(2)} \ln(p^{(1)}_i p^{(2)}_j)\\
                                     ...
                                     &= S(\rho^{(1)}) + S(\rho^{(2)}).
\end{align}
Assim, a aditividade da entropia de von Neumann de sistemas sem correlação é familiar à extensividade da entropia na Termodinâmica.

\section{Estados de mistura e evolução unitária}
Uma matriz densidade \(\rho\) representa um estado puro se tem um único autovalor igual a 1. Essa condição é equivalente à entropia de von Neumann se anular. De fato, da expressão
\begin{equation}
    S(\rho) = -\sum_i p_i \ln p_i,
\end{equation}
...
\begin{equation}
    S(\rho) \geq 0,
\end{equation}
onde a igualdade vale se e somente se \(\rho\) é um estado puro.
...
\end{equation}
para qualquer operador unitário \(U\). Em particular, a entropia de von Neumann é constante no tempo para sistemas fechados, e, portanto, a evolução temporal unitária leva estados puros em estados puros e estados de mistura em estados de mistura.
...
\begin{equation}
    \entropy{\rho}{\sigma} = \Tr \rho \ln \rho - \Tr \rho \ln \sigma.
\end{equation}
Esse funcional satisfaz\cite{breuer}
\begin{equation}
    \entropy{\rho}{\sigma} \geq 0,
\end{equation}
\begin{equation}
    \entropy{U \rho \herm{U}}{U\sigma \herm{U}} = \entropy{\rho}{\sigma},
\end{equation}

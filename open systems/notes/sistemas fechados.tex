% vim: spl=pt
\chapter{Evolução temporal de sistemas fechados}
\nocite{breuer}
...
\begin{equation}
    \diff{\rho(t)}{t} = -i\commutator{H(t)}{\rho(t)} = \mathcal{L}(t)\left\{\rho(t)\right\}
\end{equation}
onde \(\mathcal{L}(t)\) é o operador linear agindo no espaço de operadores dado por \(\mathcal{L}(t)\left\{A\right\} = -i\commutator{H(t)}{A}\). Podemos escrever a solução formal
\begin{equation}
    \rho(t) = \texpl\left[\int_{t_0}^t\dli{s} \mathcal{L}(t)\right]\left\{\rho(t_0)\right\},
    \label{eq:liouville}
\end{equation}
...
\begin{align}
    \rho(t) &= \exp\left[(t-t_0) \mathcal{L}\right]\left\{\rho(t_0)\right\}\\
...
\section{Representação de interação}
...
\begin{equation}
    \rho_I(t) = U_I(t, t_0) \rho(t_0) \herm{U}_I(t, t_0) = \herm{U}_0(t,t_0) \rho(t) U_0(t,t_0),
\end{equation}
de forma que
\begin{equation}
